...
export function Survey() {
  ...
  const [formData, setFormData] = useState({
    ...
    hasSensitivity: userProfile.hasSensitivity || false,
  ...
  const handleContinue = () => {
    if (step < 3) {
      ...
      if (returnTo === 'personalized') {
        navigate('/products?personalized=true');
      } else if (returnTo === 'compatibility') {
        navigate('/');
      } else {
        navigate(-1);
      }
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
          ...
          {[1, 2, 3].map(i => (
            <div
              ...
              className={`h-1 flex-1 rounded-full ${
                i <= step ? 'bg-gradient-to-r from-[#D4C5E8] via-[#F5E8DD] to-[#FFFEF9]' : 'bg-gray-200'
              }`}
            />
          ))}
        </div>
                  ...
                  <button
                    ...
                    className={`py-3 px-4 rounded-xl border-2 text-sm font-medium transition-all ${
                      formData.ageRange === range
                        ? 'border-transparent bg-gradient-to-br from-[#D4C5E8] via-[#F5E8DD] to-[#FFFEF9] text-gray-900'
      ...
      {/* Footer */}
      <div className="px-6 pb-12 pt-4 border-t border-gray-200">
        <button
          onClick={handleContinue}
          disabled={!canContinue()}
          className="w-full bg-gradient-to-br from-[#D4C5E8] via-[#F5E8DD] to-[#FFFEF9] text-gray-900 rounded-2xl py-4 font-bold text-base flex items-center justify-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed active:scale-[0.98] transition-all shadow-lg"
        >
